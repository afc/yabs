...
/* another way to i12n and or not primitive procedure
 *  is using symbol e.g. and_symbol, or_symbol, not_symbol
 *  similair way as like if, begin, cond syntax i12n
 * */
object *and_proc(object *args)
{
    object *result = true_object;       /* (and) return #t */
    while (!is_empty_list(args)) {
        result = car(args);
        /* you cannot simply using is_false(result) to judge if result
         *  is a boolean and its value is false  since (and)'s arguments
         *  are evaluated via list_of_values() which using make_cons()
         *  so the newly created #f-boolean obj. is differ from false_object
         * another way is to rewrite is_false() for a generous purpose
         * */
...
void output(object *obj)
{
    ...
    switch (obj->type) {
        ...
        case COMPOUND_PROC:
             ...
             *  printf("#<procedure %s>", sym->data.symbol.value);
             * */
            printf("#<procedure>");
            break;
